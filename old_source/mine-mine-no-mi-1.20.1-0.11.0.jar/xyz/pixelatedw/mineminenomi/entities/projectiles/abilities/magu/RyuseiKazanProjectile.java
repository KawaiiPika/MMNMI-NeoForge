...
public class RyuseiKazanProjectile extends NuProjectileEntity {
   public RyuseiKazanProjectile(EntityType<? extends RyuseiKazanProjectile> type, Level world) {
      super(type, world);
   }

   public RyuseiKazanProjectile(Level world, LivingEntity player, IAbility ability) {
      super((EntityType)ModProjectiles.RYUSEI_KAZAN.get(), world, player, ability);
   ...
   private void entityHitEvent(EntityHitResult result) {
      ...
      if (var3 instanceof LivingEntity target) {
         AbilityHelper.setSecondsOnFireBy(target, 8, this.getOwner());
      }
