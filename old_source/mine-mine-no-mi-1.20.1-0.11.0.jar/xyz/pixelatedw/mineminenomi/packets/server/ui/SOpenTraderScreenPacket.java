...
public class SOpenTraderScreenPacket {
   ...
   public SOpenTraderScreenPacket(int traderEntity, List<TradeEntry> tradeEntries) {
      this.traderEntity = traderEntity;
      this.tradeEntries = tradeEntries;
   }

   public SOpenTraderScreenPacket(int traderEntity, List<TradeEntry> tradeEntries, int vearthTrades) {
      this.traderEntity = traderEntity;
      this.tradeEntries = tradeEntries;
   ...
   public static SOpenTraderScreenPacket decode(FriendlyByteBuf buffer) {
   ...
   public static void handle(SOpenTraderScreenPacket message, Supplier<NetworkEvent.Context> ctx) {
      if (((NetworkEvent.Context)ctx.get()).getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
         ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> SOpenTraderScreenPacket.ClientHandler.handle(message));
      }
   ...
   public static class ClientHandler {
      ...
      public static void handle(SOpenTraderScreenPacket message) {
         Entity trader = Minecraft.m_91087_().f_91073_.m_6815_(message.traderEntity);
         if (trader instanceof TraderEntity traderEntity) {
            traderEntity.setTradingItems(message.tradeEntries);
            if (traderEntity instanceof SkypieanTraderEntity skypeanTrader) {
               skypeanTrader.setTradesLeft(message.vearthTrades);
            }

            Minecraft.m_91087_().m_91152_(new TraderScreen(traderEntity));
         }
