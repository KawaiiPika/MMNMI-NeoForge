...
public class SStartDisablePacket {
   ...
   public static SStartDisablePacket decode(FriendlyByteBuf buffer) {
   ...
   public static void handle(SStartDisablePacket message, Supplier<NetworkEvent.Context> ctx) {
      if (((NetworkEvent.Context)ctx.get()).getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
         ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> SStartDisablePacket.ClientHandler.handle(message));
      }
      ...
      public static void handle(SStartDisablePacket message) {
         ...
         if (target != null && target instanceof LivingEntity entity) {
            ...
            if (props != null) {
               ...
               if (abl != null) {
                  abl.getComponent((AbilityComponentKey)ModAbilityComponents.DISABLE.get()).ifPresent((c) -> c.startDisable(entity, message.disableTime));
               }
            }
         }
      }
