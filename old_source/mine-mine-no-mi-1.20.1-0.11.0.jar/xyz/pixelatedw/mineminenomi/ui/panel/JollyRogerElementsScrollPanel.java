...
public class JollyRogerElementsScrollPanel extends ScrollPanel {
   ...
   public JollyRogerElementsScrollPanel(JollyRogerEditorScreen parent, int listWidth, int elementsPerRow) {
      super(parent.getMinecraft(), listWidth, parent.f_96544_, 0, parent.f_96543_ - listWidth);
      this.parent = parent;
      ...
      for(int row = 0; row < this.rows; ++row) {
         ...
         for(Entry elemEntry : this.entries.stream().skip((long)skips).limit((long)this.elementsPerRow).toList()) {
            ...
            if (elemEntry.element != null) {
               ...
               rect.draw(graphics.m_280168_(), (float)(x - 1 - hoveredX), (float)(y - 4 - hoveredY));
               Component localizedName = elemEntry.element.getLocalizedName();
               int centerX = x - this.parent.getMinecraft().f_91062_.m_92852_(localizedName) / 2;
               RendererHelper.drawStringWithBorder(this.parent.getMinecraft().f_91062_, graphics, localizedName, centerX + 25, y + 42, textColor);
            } else if (elemEntry.equals(REMOVE_ELEMENT)) {
               iconSize *= 0.8F;
               TexturedRectUI rect = new TexturedRectUI(ModResources.EXPIRED);
               rect.setScale(iconSize);
               rect.draw(graphics.m_280168_(), (float)(x + 3 - hoveredX), (float)(y - hoveredY));
               Component localizedName = ModI18n.GUI_REMOVE;
               int centerX = x - this.parent.getMinecraft().f_91062_.m_92852_(localizedName) / 2;
               RendererHelper.drawStringWithBorder(this.parent.getMinecraft().f_91062_, graphics, localizedName, centerX + 24, y + 42, textColor);
            }
