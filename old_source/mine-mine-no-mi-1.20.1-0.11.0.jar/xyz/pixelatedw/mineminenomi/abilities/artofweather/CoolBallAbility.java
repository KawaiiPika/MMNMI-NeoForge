...
public class CoolBallAbility extends WeatherBallAbility {
   public static final RegistryObject<AbilityCore<CoolBallAbility>> INSTANCE = ModRegistry.registerAbility("cool_ball", "Cool Ball", (id, name) -> {
      Component[] desc = AbilityHelper.registerDescriptionText("mineminenomi", id, ImmutablePair.of("Launch a Cool Ball from your Clima Tact to use for different Tempos", (Object)null), ImmutablePair.of("§aSHIFT-USE§r: Loads the ball into the clima tact", (Object)null));
      return (new AbilityCore.Builder(id, name, AbilityCategory.STYLE, CoolBallAbility::new)).addDescriptionLine(desc).setSourceElement(SourceElement.ICE).setSourceHakiNature(SourceHakiNature.IMBUING).setNodeFactories(CoolBallAbility::createNode).build("mineminenomi");
   });
   ...
   public WeatherBallProjectile getWeatherBallEntity(LivingEntity entity, Vec3 lookVec) {
      CoolBallProjectile proj = new CoolBallProjectile(entity.m_9236_(), entity, this);
   ...
   private static AbilityNode createNode(LivingEntity entity) {
      AbilityNode node = new AbilityNode(((AbilityCore)INSTANCE.get()).getLocalizedName(), ((AbilityCore)INSTANCE.get()).getIcon(), new AbilityNode.NodePos(-10.0F, 1.0F));
