...
public class BusoshokuHakiEmissionAbility extends PunchAbility {
   public static final RegistryObject<AbilityCore<BusoshokuHakiEmissionAbility>> INSTANCE = ModRegistry.registerAbility("busoshoku_haki_emission", "Busoshoku Haki: Emission", (id, name) -> {
      Component[] desc = AbilityHelper.registerDescriptionText("mineminenomi", id, ImmutablePair.of("Allows the user to emit their haki over a short distance without a medium.", (Object)null));
      return (new AbilityCore.Builder(id, name, AbilityCategory.HAKI, BusoshokuHakiEmissionAbility::new)).addDescriptionLine(desc).setSourceType(SourceType.FIST).setUnlockCheck(BusoshokuHakiEmissionAbility::canUnlock).build("mineminenomi");
   });
   ...
   private void onContinuityStart(LivingEntity entity, IAbility ability) {
      this.statsComponent.addAttributeModifier(ForgeMod.ENTITY_REACH, HakiHelper.getAdvancedBusoshokuHakiAttackReachBonus((AbilityCore)INSTANCE.get(), (double)getReachBonus(entity)));
   ...
   public static float getReachBonus(LivingEntity player) {
      ...
      if (hakiProps == null) {
         ...
         float hakiMultiplier = hakiProps.getBusoshokuHakiExp() / (float)ServerConfig.getHakiExpLimit();
   ...
   private static boolean canUnlock(LivingEntity user) {
      ...
      if (abilityProps != null && hakiProps != null && statsProps != null) {
         boolean hasBusoUnlocked = abilityProps.hasUnlockedAbility((AbilityCore)BusoshokuHakiHardeningAbility.INSTANCE.get()) || abilityProps.hasUnlockedAbility((AbilityCore)BusoshokuHakiImbuingAbility.INSTANCE.get());
         boolean hasBusoExp = (double)hakiProps.getBusoshokuHakiExp() > HakiHelper.getBusoshokuEmissionExpNeeded(user);
         return hasBusoUnlocked && statsProps.getDoriki() > (double)6000.0F && hasBusoExp;
      } else {
         return false;
      }
   }
   ...
   public boolean onHitEffect(LivingEntity entity, LivingEntity target, DamageSource source) {
      ...
      WyHelper.spawnParticleEffect((ParticleEffect)ModParticleEffects.EMISSION_BURST.get(), entity, entity.m_20185_(), entity.m_20188_(), entity.m_20189_());
      target.m_147240_((double)4.0F, entity.m_20185_() - target.m_20185_(), entity.m_20189_() - target.m_20189_());
      return true;
   }
   ...
   static {
      OVERLAY = (new AbilityOverlay.Builder()).setOverlayPart(AbilityOverlay.OverlayPart.LIMB).setColor(new Color(226, 129, 25, 40)).build();
   }
}
