...
import xyz.pixelatedw.mineminenomi.abilities.nitoryu.NanajuniPoundHoAbility;
...
public class HyakuhachiPoundHoAbility extends Ability {
   ...
   public static final RegistryObject<AbilityCore<HyakuhachiPoundHoAbility>> INSTANCE = ModRegistry.registerAbility("hyakuhachi_pound_ho", "Hyakuhachi Pound Ho", (id, name) -> {
      Component[] desc = AbilityHelper.registerDescriptionText("mineminenomi", id, ImmutablePair.of("Same principle as the thirty-six version of the same attack. Holding their two swords horizontally above the shoulder and the other in their mouth in the same direction, and then performs a circular swing that launches three air compressed projectiles spiraling towards the target instead of one or two, tripling the power of the technique.", (Object)null));
      return (new AbilityCore.Builder(id, name, AbilityCategory.STYLE, HyakuhachiPoundHoAbility::new)).addDescriptionLine(desc).addAdvancedDescriptionLine(AbilityDescriptionLine.NEW_LINE, CooldownComponent.getTooltip(320.0F)).addAdvancedDescriptionLine(ProjectileComponent.getProjectileTooltips()).setSourceHakiNature(SourceHakiNature.IMBUING).setSourceType(SourceType.SLASH).setSourceElement(SourceElement.AIR).setNodeFactories(HyakuhachiPoundHoAbility::createNode).build("mineminenomi");
   });
   private final AnimationComponent animationComponent = new AnimationComponent(this);
   private final ProjectileComponent projectileComponent = new ProjectileComponent(this, this::createProjectile);
   private final ExplosionComponent explosionComponent = new ExplosionComponent(this);

   public HyakuhachiPoundHoAbility(AbilityCore<HyakuhachiPoundHoAbility> core) {
      super(core);
   ...
   private HyakuhachiPoundHoProjectile createProjectile(LivingEntity entity) {
      HyakuhachiPoundHoProjectile proj = new HyakuhachiPoundHoProjectile(entity.m_9236_(), entity, this);
      proj.setDamage(30.0F);
      return proj;
   }

   private static AbilityNode createNode(LivingEntity entity) {
      AbilityNode hyakuhachiPoundHo = new AbilityNode(((AbilityCore)INSTANCE.get()).getLocalizedName(), ((AbilityCore)INSTANCE.get()).getIcon(), new AbilityNode.NodePos(13.0F, -6.0F));
      ...
      hyakuhachiPoundHo.addPrerequisites(((AbilityCore)NanajuniPoundHoAbility.INSTANCE.get()).getNode(entity));
      hyakuhachiPoundHo.setUnlockRule(unlockCondition, unlockAction);
      return hyakuhachiPoundHo;
   }
}
