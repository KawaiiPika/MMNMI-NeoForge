...
public class YomiReviveTrigger extends SimpleCriterionTrigger<Instance> {
   private static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath("mineminenomi", "yomi_revive");
   ...
   public void trigger(ServerPlayer player) {
      this.m_66234_(player, (inst) -> {
         IDevilFruit props = (IDevilFruit)DevilFruitCapability.get(player).orElse((Object)null);
         if (props == null) {
            return false;
         } else {
            return props.hasDevilFruit(ModFruits.YOMI_YOMI_NO_MI) && !((MorphInfo)ModMorphs.YOMI_SKELETON.get()).isActive(player);
         }
      });
   }
   ...
   public static class Instance extends AbstractCriterionTriggerInstance {
      public Instance(ContextAwarePredicate entityPredicate) {
         super(YomiReviveTrigger.ID, entityPredicate);
      }
