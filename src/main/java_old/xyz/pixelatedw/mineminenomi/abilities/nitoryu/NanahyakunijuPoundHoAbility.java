...
public class NanahyakunijuPoundHoAbility extends Ability {
   ...
   public static final RegistryObject<AbilityCore<NanahyakunijuPoundHoAbility>> INSTANCE = ModRegistry.registerAbility("nanahyakuniju_pound_ho", "Nanahyakuniju Pound Ho", (id, name) -> {
      Component[] desc = AbilityHelper.registerDescriptionText("mineminenomi", id, ImmutablePair.of("A more powerful version of the Nanajuni Pound Ho. Holding their two swords horizontally above the shoulder, the user then performs a circular swing that launches two air compressed projectiles spiraling towards the target.", (Object)null));
      return (new AbilityCore.Builder(id, name, AbilityCategory.STYLE, NanahyakunijuPoundHoAbility::new)).addDescriptionLine(desc).addAdvancedDescriptionLine(AbilityDescriptionLine.NEW_LINE, CooldownComponent.getTooltip(320.0F)).addAdvancedDescriptionLine(ProjectileComponent.getProjectileTooltips()).setSourceHakiNature(SourceHakiNature.IMBUING).setSourceType(SourceType.SLASH).setSourceElement(SourceElement.AIR).setNodeFactories(NanahyakunijuPoundHoAbility::createNode).build("mineminenomi");
   });
   ...
   private NanahyakunijuPoundHoProjectile createProjectile(LivingEntity entity) {
      NanahyakunijuPoundHoProjectile proj = new NanahyakunijuPoundHoProjectile(entity.m_9236_(), entity, this);
      proj.setDamage(30.0F);
      return proj;
   }

   private static AbilityNode createNode(LivingEntity entity) {
      AbilityNode nanahyakunijuPoundHo = new AbilityNode(((AbilityCore)INSTANCE.get()).getLocalizedName(), ((AbilityCore)INSTANCE.get()).getIcon(), new AbilityNode.NodePos(13.0F, -10.0F));
      ...
      nanahyakunijuPoundHo.addPrerequisites(((AbilityCore)SanbyakurokujuPoundHoAbility.INSTANCE.get()).getNode(entity));
      nanahyakunijuPoundHo.setUnlockRule(unlockCondition, unlockAction);
      return nanahyakunijuPoundHo;
   }
}
