...
public class TokuyoAburaBoshiAbility extends Ability {
   ...
   public static final RegistryObject<AbilityCore<TokuyoAburaBoshiAbility>> INSTANCE = ModRegistry.registerAbility("tokuyo_abura_boshi", "Tokuyo Abura Boshi", (id, name) -> {
      Component[] desc = AbilityHelper.registerDescriptionText("mineminenomi", id, ImmutablePair.of("Fires a pellet that upon impact releases oil coating the target in it and making the ground slippery", (Object)null));
      return (new AbilityCore.Builder(id, name, AbilityCategory.STYLE, TokuyoAburaBoshiAbility::new)).addDescriptionLine(desc).addAdvancedDescriptionLine(AbilityDescriptionLine.NEW_LINE, CooldownComponent.getTooltip(140.0F), ContinuousComponent.getTooltip()).addAdvancedDescriptionLine(ProjectileComponent.getProjectileTooltips()).setSourceHakiNature(SourceHakiNature.IMBUING).setSourceType(SourceType.BULLET).setNodeFactories(TokuyoAburaBoshiAbility::createNode).build("mineminenomi");
   });
   ...
   public TokuyoAburaBoshiProjectile createProjectile(LivingEntity entity) {
      TokuyoAburaBoshiProjectile proj = new TokuyoAburaBoshiProjectile(entity.m_9236_(), entity, this);
      return proj;
   }

   private static AbilityNode createNode(LivingEntity entity) {
      AbilityNode node = new AbilityNode(((AbilityCore)INSTANCE.get()).getLocalizedName(), ((AbilityCore)INSTANCE.get()).getIcon(), new AbilityNode.NodePos(-6.0F, -11.0F));
