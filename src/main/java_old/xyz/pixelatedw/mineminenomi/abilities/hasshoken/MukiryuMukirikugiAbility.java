...
public class MukiryuMukirikugiAbility extends PunchAbility {
   ...
   public static final RegistryObject<AbilityCore<MukiryuMukirikugiAbility>> INSTANCE = ModRegistry.registerAbility("mukiryu_mukirikugi", "Mukiryu Mukirikugi", (id, name) -> {
      Component[] desc = AbilityHelper.registerDescriptionText("mineminenomi", id, ImmutablePair.of("The user focuses their body's vibrations into their next attack to enhance the damage.", (Object)null));
      return (new AbilityCore.Builder(id, name, AbilityCategory.STYLE, MukiryuMukirikugiAbility::new)).addDescriptionLine(desc).addAdvancedDescriptionLine(AbilityDescriptionLine.NEW_LINE, CooldownComponent.getTooltip(700.0F), ContinuousComponent.getTooltip(), ChangeStatsComponent.getTooltip()).setSourceHakiNature(SourceHakiNature.HARDENING).setSourceElement(SourceElement.SHOCKWAVE).setSourceType(SourceType.FIST, SourceType.INTERNAL).setNodeFactories(MukiryuMukirikugiAbility::createNode).setUnlockCheck(MukiryuMukirikugiAbility::canUnlock).build("mineminenomi");
   });
   private final ProjectileComponent projectileComponent = new ProjectileComponent(this, this::createProjectile);
   private final ExplosionComponent explosionComponent = new ExplosionComponent(this);
   private final PoolComponent poolComponent;

   public MukiryuMukirikugiAbility(AbilityCore<MukiryuMukirikugiAbility> core) {
      super(core);
   ...
   private RokuoganProjectile createProjectile(LivingEntity entity) {
      ...
      proj.addBlockHitEvent(100, (result) -> {
         ...
         ExplosionComponent.createExplosion((NuProjectileEntity)proj, (comp) -> {
            AbilityExplosion explosion = comp.createExplosion(proj, entity, (double)hit.m_123341_(), (double)hit.m_123342_(), (double)hit.m_123343_(), 5.0F);
            explosion.setStaticDamage(5.0F);
            explosion.setSmokeParticles((ParticleEffect)ModParticleEffects.COMMON_EXPLOSION.get(), new CommonExplosionParticleEffect.Details((double)5.0F));
            explosion.m_46061_();
         });
      });
      return proj;
   ...
   private static AbilityNode createNode(LivingEntity entity) {
      AbilityNode mukiryuMukirikugi = new AbilityNode(((AbilityCore)INSTANCE.get()).getLocalizedName(), ((AbilityCore)INSTANCE.get()).getIcon(), new AbilityNode.NodePos(-19.0F, 0.0F));
      NodeUnlockCondition unlockCondition = DorikiUnlockCondition.atLeast((double)9000.0F).and(TrainingPointsUnlockCondition.martialArts((double)50.0F));
      NodeUnlockAction unlockAction = SubtractPointsAction.martialArts(50).andThen(UnlockAbilityAction.unlock(INSTANCE));
      mukiryuMukirikugi.addPrerequisites(((AbilityCore)HasshokenPassiveBonusesAbility.INSTANCE.get()).getNode(entity));
      mukiryuMukirikugi.setUnlockRule(unlockCondition, unlockAction);
      return mukiryuMukirikugi;
   }
