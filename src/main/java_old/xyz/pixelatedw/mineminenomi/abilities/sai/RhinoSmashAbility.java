...
public class RhinoSmashAbility extends RunningSmashAbility {
   public static final RegistryObject<AbilityCore<RhinoSmashAbility>> INSTANCE = ModRegistry.registerAbility("rhino_smash", "Rhino Smash", (id, name) -> {
      ...
      return (new AbilityCore.Builder(id, name, AbilityCategory.DEVIL_FRUITS, AbilityType.PASSIVE, RhinoSmashAbility::new)).addDescriptionLine(desc).addDescriptionLine(AbilityDescriptionLine.NEW_LINE, AbilityTooltipsHelper.getRequiredMorphTooltip(ModMorphs.SAI_WALK)).addAdvancedDescriptionLine(AbilityDescriptionLine.NEW_LINE, DealDamageComponent.getTooltip(2.0F), RangeComponent.getTooltip(1.5F, RangeComponent.RangeType.LINE)).setSourceHakiNature(SourceHakiNature.HARDENING).setSourceType(SourceType.FIST).build("mineminenomi");
   });

   public RhinoSmashAbility(AbilityCore<RhinoSmashAbility> core) {
      super(core, 1.5F, 4.0F);
