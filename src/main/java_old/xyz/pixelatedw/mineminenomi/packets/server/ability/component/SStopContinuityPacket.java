...
public class SStopContinuityPacket {
   ...
   public static SStopContinuityPacket decode(FriendlyByteBuf buffer) {
      SStopContinuityPacket msg = new SStopContinuityPacket();
   ...
   public static void handle(SStopContinuityPacket message, Supplier<NetworkEvent.Context> ctx) {
      if (((NetworkEvent.Context)ctx.get()).getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
         ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> SStopContinuityPacket.ClientHandler.handle(message));
      }
      ...
      public static void handle(SStopContinuityPacket message) {
         ...
         if (target != null && target instanceof LivingEntity entity) {
            ...
            if (props != null) {
               ...
               if (abl != null) {
                  abl.getComponent((AbilityComponentKey)ModAbilityComponents.CONTINUOUS.get()).ifPresent((c) -> c.stopContinuity(entity));
               }
            }
         }
      }
