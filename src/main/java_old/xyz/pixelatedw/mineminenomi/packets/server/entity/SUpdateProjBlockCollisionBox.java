...
public class SUpdateProjBlockCollisionBox {
   ...
   public static SUpdateProjBlockCollisionBox decode(FriendlyByteBuf buffer) {
   ...
   public static void handle(SUpdateProjBlockCollisionBox message, Supplier<NetworkEvent.Context> ctx) {
      if (((NetworkEvent.Context)ctx.get()).getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
         ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> SUpdateProjBlockCollisionBox.ClientHandler.handle(message));
      }
