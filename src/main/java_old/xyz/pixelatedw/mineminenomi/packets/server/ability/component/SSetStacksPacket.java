...
public class SSetStacksPacket {
   ...
   public void encode(FriendlyByteBuf buffer) {
      ...
      buffer.writeInt(this.stacks);
   }

   public static SSetStacksPacket decode(FriendlyByteBuf buffer) {
      SSetStacksPacket msg = new SSetStacksPacket();
      msg.entityId = buffer.readInt();
      msg.abilityId = buffer.readInt();
      msg.stacks = buffer.readInt();
      return msg;
   }

   public static void handle(SSetStacksPacket message, Supplier<NetworkEvent.Context> ctx) {
      if (((NetworkEvent.Context)ctx.get()).getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
         ((NetworkEvent.Context)ctx.get()).enqueueWork(() -> SSetStacksPacket.ClientHandler.handle(message));
      }
      ...
      public static void handle(SSetStacksPacket message) {
         ...
         if (target != null && target instanceof LivingEntity entity) {
            ...
            if (props != null) {
               ...
               if (abl != null) {
                  abl.getComponent((AbilityComponentKey)ModAbilityComponents.STACK.get()).ifPresent((c) -> c.setStacks(entity, abl, message.stacks));
               }
            }
         }
      }
