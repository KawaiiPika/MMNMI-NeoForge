package xyz.pixelatedw.mineminenomi.client.ui;
...
public class TexturedRectUI {
    ...
    public void draw(PoseStack matrixStack, float x, float y) {
        float uAtlasScale = (this.flipX ? -1.0F : 1.0F) / 256.0F;
        ...
        BufferBuilder wr = tessellator.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX_COLOR);
        Matrix4f matrix = matrixStack.last().pose();
        
        wr.addVertex(matrix, x, y + height2, this.zLevel).setColor(this.red, this.green, this.blue, this.alpha).setUv(this.u * uAtlasScale, (this.v + this.height) * vAtlasScale);
        wr.addVertex(matrix, x + width2, y + height2, this.zLevel).setColor(this.red, this.green, this.blue, this.alpha).setUv((this.u + this.width) * uAtlasScale, (this.v + this.height) * vAtlasScale);
        wr.addVertex(matrix, x + width2, y, this.zLevel).setColor(this.red, this.green, this.blue, this.alpha).setUv((this.u + this.width) * uAtlasScale, this.v * vAtlasScale);
        wr.addVertex(matrix, x, y, this.zLevel).setColor(this.red, this.green, this.blue, this.alpha).setUv(this.u * uAtlasScale, this.v * vAtlasScale);
