...
public class EmissionBurstParticleEffect extends Particle {

    protected EmissionBurstParticleEffect(ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
        super(level, x, y, z, xSpeed, ySpeed, zSpeed);
        this.xd = xSpeed;
        this.yd = ySpeed;
        this.zd = zSpeed;
        this.lifetime = 10;
        this.bbWidth = 8.0F; // Based on old logic
    }
    ...
    public void render(VertexConsumer buffer, Camera camera, float partialTicks) {
        ...
        // Visual simulation of Emission Burst
        for (int i = 0; i < 5; i++) {
            Vec3 moveVec = new Vec3(level.random.nextDouble() * 0.12, level.random.nextDouble() * 0.12, level.random.nextDouble() * 0.12);
    ...
    public static class Provider implements ParticleProvider<SimpleParticleData> {
        ...
        public Particle createParticle(SimpleParticleData type, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new EmissionBurstParticleEffect(level, x, y, z, xSpeed, ySpeed, zSpeed);
        }
    }
}
