...
public abstract class Objective {
    ...
    private final PriorityEventPool<IStartObjective> startEvents = new PriorityEventPool<>();
    private final PriorityEventPool<IRestartObjective> restartEvents = new PriorityEventPool<>();
    private final PriorityEventPool<ICompleteObjective> completeEvents = new PriorityEventPool<>();
    ...
    public void setProgress(Player player, float progress, boolean force) {
        if (!this.isLocked()) {
            if (!this.isComplete() || force) {
                float previousProgress = this.getProgress();
                this.progress = Mth.clamp(progress, 0.0F, this.getMaxProgress());
                if (this.isComplete()) {
                    if (this.parent.tryComplete(player)) {
                        ...
                        // Ping logic removed for now, needs networking implementation
                    }
                }
            }
        }
