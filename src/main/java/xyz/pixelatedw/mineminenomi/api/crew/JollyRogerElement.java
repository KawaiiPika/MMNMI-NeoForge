...
public class JollyRogerElement {
   ...
   public JollyRogerElement(LayerType type) {
      this.localizedName = Component.empty();
   ...
   // Dummy as registry isn't fully ported yet
   public static JollyRogerElement of(ResourceLocation res) {
      return new JollyRogerElement(LayerType.BASE, res);
   }
   ...
   public JollyRogerElement setColor(int red, int green, int blue) {
      ...
      this.green = green;
      this.blue = blue;
      // Dummy rgbToInt to avoid WyHelper dependency if missing
      this.color = ((255 & 0xFF) << 24) | ((red & 0xFF) << 16) | ((green & 0xFF) << 8) | (blue & 0xFF);
      this.fullColor = new Color(this.color);
   ...
   public TexturedRectUI getTexturedRect() {
      // Texture comparison was breaking, recreating is safe enough for dummy logic
   ...
   public void write(CompoundTag nbt) {
      ...
      nbt.putBoolean("isFlippedVertically", this.isFlippedVertically);
      nbt.putBoolean("isFlippedHorizontally", this.isFlippedHorizontally);
   }

   public void read(CompoundTag nbt) {
      int color = nbt.getInt("color");
      this.setColor(color);
      this.isFlippedVertically = nbt.getBoolean("isFlippedVertically");
      this.isFlippedHorizontally = nbt.getBoolean("isFlippedHorizontally");
   }

   public @Nullable ResourceLocation getRegistryKey() {
       // Dummy for registry logic out of scope
      return this.texture;
   }
