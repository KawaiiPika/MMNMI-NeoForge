...
public class DopingAbility extends Ability {
    ...
    protected void startUsing(LivingEntity entity) {
        if (!entity.level().isClientSide) {
            ...
            target.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 300, 1));
            target.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 300, 1));
            entity.sendSystemMessage(Component.literal("Doped " + target.getDisplayName().getString()));
        }
    ...
    public Component getDisplayName() {
        return Component.literal("Doping");
    }
}
