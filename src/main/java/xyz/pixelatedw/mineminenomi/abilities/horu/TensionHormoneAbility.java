...
public class TensionHormoneAbility extends Ability {
    ...
    protected void startUsing(LivingEntity entity) {
        if (!entity.level().isClientSide) {
            if (entity.isShiftKeyDown()) {
                entity.addEffect(new MobEffectInstance(ModEffects.TENSION_HORMONE, 200, 1));
            ...
            Entity target = trace != null && !trace.isEmpty() ? trace.get(0) : null;
                if (target instanceof LivingEntity livingTarget) {
                    livingTarget.addEffect(new MobEffectInstance(ModEffects.TENSION_HORMONE, 200, 1));
                    this.startCooldown(entity, 400);
                }
            }
        }
    ...
    public Component getDisplayName() {
        return Component.translatable("ability.mineminenomi.tension_hormone");
    }
}
