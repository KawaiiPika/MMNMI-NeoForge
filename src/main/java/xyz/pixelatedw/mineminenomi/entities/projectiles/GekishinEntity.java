...
public class GekishinEntity extends ThrowableItemProjectile {
    private int life = 0;
    private final int maxLife = 40;

    public GekishinEntity(EntityType<? extends GekishinEntity> type, Level world) {
        super(type, world);
    }

    public GekishinEntity(Level world, LivingEntity owner) {
        super(ModEntities.GEKISHIN.get(), owner, world);
    }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (result.getEntity() instanceof LivingEntity target && this.getOwner() instanceof LivingEntity owner) {
            ...
            target.setDeltaMovement(this.getDeltaMovement().scale(1.5).add(0, 0.4, 0));
