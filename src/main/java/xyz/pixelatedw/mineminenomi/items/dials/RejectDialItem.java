   ...
   public boolean hurtEnemy(ItemStack itemStack, LivingEntity target, LivingEntity attacker) {
      if (!attacker.level().isClientSide && attacker instanceof Player playerAttacker) {
         if (playerAttacker.getCooldowns().getCooldownPercent(this, 0.0F) > 0.0F) {
            ...
            playerAttacker.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 600, 2, false, false));
            playerAttacker.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 600, 1, false, false));
            playerAttacker.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 600, 1, false, false));
            DamageSource source = playerAttacker.damageSources().magic();
            if (!playerAttacker.isCreative()) {
               attacker.hurt(source, attacker.getHealth() - 1.0F);
            }
