...
content = content.replace("xyz.pixelatedw.mineminenomi.init.ModRecipes.SMITHING_DIAL.get()", "new SmithingDialRecipe.Serializer()")
...
content2 = content2.replace("xyz.pixelatedw.mineminenomi.init.ModRecipes.SMITHING_ENCHANTMENT.get()", "new SmithingEnchantmentRecipe.Serializer()")
...
content3 = content3.replace("xyz.pixelatedw.mineminenomi.init.ModRecipes.MULTI_CHANNEL_ARMOR_DYE.get()", "new net.minecraft.world.item.crafting.SimpleCraftingRecipeSerializer<>(MultiChannelDyeRecipe::new)")

with open('src/main/java/xyz/pixelatedw/mineminenomi/api/crafting/MultiChannelDyeRecipe.java', 'w') as f:
    f.write(content3)

# And now, remove them from ModRecipeProvider to just completely verify it's the serializers
